...
define('READINIZER_PRO_VERSION', '2.0.0');
...
class ReadinizerPro {
    ...
    private function init_options() {
        $this->options = get_option('readinizer_pro_options', array(
            'enabled' => true,
            'position' => 'before',
    ...
    public function enqueue_scripts() {
        ...
        if ($this->options['enable_progress_bars']) {
            ...
            wp_localize_script('readinizer-pro-progress', 'readinizerPro', array(
    ...
    public function enqueue_admin_scripts($hook) {
        ...
        wp_localize_script('readinizer-pro-admin-js', 'readinizerProAdmin', array(
            'ajaxurl' => admin_url('admin-ajax.php'),
            'nonces' => array(
    ...
    public function admin_init() {
        ...
        add_settings_section(
            'readinizer_pro_general_section',
            __('General Settings', 'readinizer-pro'),
            array($this, 'general_section_callback'),
            'readinizer-pro'
        );

        add_settings_section(
            'readinizer_pro_display_section',
            __('Display Settings', 'readinizer-pro'),
            array($this, 'display_section_callback'),
            'readinizer-pro'
        );

        add_settings_section(
            'readinizer_pro_style_section',
            __('Style Settings', 'readinizer-pro'),
            array($this, 'style_section_callback'),
            'readinizer-pro'
        );

        // General settings fields
        add_settings_field('enabled', __('Enable Readinizer Pro', 'readinizer-pro'), array($this, 'enabled_callback'), 'readinizer-pro', 'readinizer_pro_general_section');
        add_settings_field('post_types', __('Post Types', 'readinizer-pro'), array($this, 'post_types_callback'), 'readinizer-pro', 'readinizer_pro_general_section');
        add_settings_field('words_per_minute', __('Reading Speed (words per minute)', 'readinizer-pro'), array($this, 'words_per_minute_callback'), 'readinizer-pro', 'readinizer_pro_general_section');
        add_settings_field('show_wpezo_credit', __('Show WPezo Credit', 'readinizer-pro'), array($this, 'show_wpezo_credit_callback'), 'readinizer-pro', 'readinizer_pro_general_section');

        // Display settings fields
        add_settings_field('position', __('Display Position', 'readinizer-pro'), array($this, 'position_callback'), 'readinizer-pro', 'readinizer_pro_display_section');
        add_settings_field('show_reading_time', __('Show Reading Time', 'readinizer-pro'), array($this, 'show_reading_time_callback'), 'readinizer-pro', 'readinizer_pro_display_section');
        add_settings_field('show_word_count', __('Show Word Count', 'readinizer-pro'), array($this, 'show_word_count_callback'), 'readinizer-pro', 'readinizer_pro_display_section');
        add_settings_field('custom_text', __('Custom Text Template', 'readinizer-pro'), array($this, 'custom_text_callback'), 'readinizer-pro', 'readinizer_pro_display_section');

        // Style settings fields
        add_settings_field('display_style', __('Display Style', 'readinizer-pro'), array($this, 'display_style_callback'), 'readinizer-pro', 'readinizer_pro_style_section');
        add_settings_field('text_color', __('Text Color', 'readinizer-pro'), array($this, 'text_color_callback'), 'readinizer-pro', 'readinizer_pro_style_section');
        add_settings_field('background_color', __('Background Color', 'readinizer-pro'), array($this, 'background_color_callback'), 'readinizer-pro', 'readinizer_pro_style_section');
        add_settings_field('border_radius', __('Border Radius (px)', 'readinizer-pro'), array($this, 'border_radius_callback'), 'readinizer-pro', 'readinizer_pro_style_section');
        add_settings_field('font_size', __('Font Size (px)', 'readinizer-pro'), array($this, 'font_size_callback'), 'readinizer-pro', 'readinizer_pro_style_section');
    }
    ...
    public function generate_reading_time_html($stats) {
        ...
        $custom_styles = '';
        if ($this->options['display_style'] === 'custom') {
    ...
    public function add_reading_time_to_content($content) {
        ...
        } elseif ($this->options['position'] === 'after') {
    ...
    public function readinizer_shortcode($atts) {
        ...
        $this->options['show_word_count'] = ($atts['show_word_count'] === 'true');
        $this->options['show_reading_time'] = ($atts['show_reading_time'] === 'true');
        $this->options['display_style'] = sanitize_text_field($atts['style']);
        $this->options['show_wpezo_credit'] = ($atts['show_credit'] === 'true');
    ...
    public function activate() {
        ...
        // Create necessary database tables or perform setup tasks
        flush_rewrite_rules();
    }
    ...
    public function sanitize_options($input) {
        ...
        $sanitized['position'] = in_array($input['position'], array('before', 'after', 'none')) ? $input['position'] : 'before';
        ...
        $sanitized['display_style'] = in_array($input['display_style'], array('minimal', 'modern', 'badge', 'card', 'floating', 'custom')) ? $input['display_style'] : 'modern';
        ...
        $sanitized['progress_bar_style'] = in_array($input['progress_bar_style'], array('linear', 'circular', 'floating')) ? $input['progress_bar_style'] : 'linear';
        $sanitized['progress_bar_position'] = in_array($input['progress_bar_position'], array('top', 'bottom', 'floating-left', 'floating-right')) ? $input['progress_bar_position'] : 'top';
        ...
        $sanitized['progress_animation'] = in_array($input['progress_animation'], array('smooth', 'stepped', 'bouncy', 'none')) ? $input['progress_animation'] : 'smooth';
    ...
    public function general_section_callback() {
        echo '<p>' . __('Configure the basic settings for Readinizer Pro.', 'readinizer-pro') . '</p>';
    }

    public function display_section_callback() {
        echo '<p>' . __('Control how reading time and word count are displayed.', 'readinizer-pro') . '</p>';
    }

    public function style_section_callback() {
        echo '<p>' . __('Customize the appearance of the reading time display.', 'readinizer-pro') . '</p>';
    }

    // Field callbacks
    public function enabled_callback() {
        printf(
            '<input type="checkbox" id="enabled" name="readinizer_pro_options[enabled]" value="1" %s>',
            checked($this->options['enabled'], true, false)
        );
        echo '<label for="enabled">' . __('Enable automatic display of reading time and word count', 'readinizer-pro') . '</label>';
    }

    public function post_types_callback() {
        $post_types = get_post_types(array('public' => true), 'objects');
        foreach ($post_types as $post_type) {
            $checked = in_array($post_type->name, $this->options['post_types']) ? 'checked' : '';
            printf(
    ...
    public function words_per_minute_callback() {
        printf(
            '<input type="number" id="words_per_minute" name="readinizer_pro_options[words_per_minute]" value="%s" min="1" max="1000" step="1">',
            esc_attr($this->options['words_per_minute'])
        );
        echo '<p class="description">' . __('Average reading speed for calculating reading time. Default: 200 words per minute.', 'readinizer-pro') . '</p>';
    }

    public function show_wpezo_credit_callback() {
        printf(
            '<input type="checkbox" id="show_wpezo_credit" name="readinizer_pro_options[show_wpezo_credit]" value="1" %s>',
            checked($this->options['show_wpezo_credit'], true, false)
        );
        echo '<label for="show_wpezo_credit">' . __('Show "by WPezo" credit link (helps support plugin development)', 'readinizer-pro') . '</label>';
    }
    ...
    public function show_reading_time_callback() {
        printf(
            '<input type="checkbox" id="show_reading_time" name="readinizer_pro_options[show_reading_time]" value="1" %s>',
            checked($this->options['show_reading_time'], true, false)
        );
        echo '<label for="show_reading_time">' . __('Display estimated reading time', 'readinizer-pro') . '</label>';
    }

    public function show_word_count_callback() {
        printf(
            '<input type="checkbox" id="show_word_count" name="readinizer_pro_options[show_word_count]" value="1" %s>',
            checked($this->options['show_word_count'], true, false)
        );
        echo '<label for="show_word_count">' . __('Display word count', 'readinizer-pro') . '</label>';
    }

    public function custom_text_callback() {
        printf(
            '<input type="text" id="custom_text" name="readinizer_pro_options[custom_text]" value="%s" class="regular-text">',
            esc_attr($this->options['custom_text'])
        );
        echo '<p class="description">' . __('Use {time} for reading time and {words} for word count. Example: "Reading time: {time} • {words} words"', 'readinizer-pro') . '</p>';
    }
    ...
    public function text_color_callback() {
        printf(
            '<input type="text" id="text_color" name="readinizer_pro_options[text_color]" value="%s" class="color-picker">',
            esc_attr($this->options['text_color'])
        );
    }
    ...
    public function background_color_callback() {
        printf(
            '<input type="text" id="background_color" name="readinizer_pro_options[background_color]" value="%s" class="color-picker">',
            esc_attr($this->options['background_color'])
        );
    }
    ...
    public function border_radius_callback() {
        printf(
            '<input type="number" id="border_radius" name="readinizer_pro_options[border_radius]" value="%s" min="0" max="50" step="1">',
            esc_attr($this->options['border_radius'])
        );
    }
    ...
    public function font_size_callback() {
        printf(
            '<input type="number" id="font_size" name="readinizer_pro_options[font_size]" value="%s" min="8" max="32" step="1">',
            esc_attr($this->options['font_size'])
        );
    }

    /**
     * Admin page HTML with PRO tabs
     */
    public function admin_page() {
        ...
        <div class="wrap readinizer-pro-admin">
            <div class="readinizer-pro-header">
                <div class="header-content">
                    <div class="header-left">
                        ...
                        <p><?php _e('Professional Reading Time & Analytics for WordPress', 'readinizer-pro'); ?></p>
                        <div class="wpezo-brand">
                            <?php printf(__('Powered by %s', 'readinizer-pro'), '<a href="' . READINIZER_PRO_BRAND_URL . '" target="_blank" class="wpezo-link">WPezo</a>'); ?>
                        </div>
                    </div>
                    <div class="header-right">
                        <div class="readinizer-pro-version">
                            <?php printf(__('Version %s', 'readinizer-pro'), READINIZER_PRO_VERSION); ?>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="nav-tab-wrapper">
                <a href="#general" class="nav-tab nav-tab-active" data-tab="general">⚙️ <?php _e('General', 'readinizer-pro'); ?></a>
                <a href="#display" class="nav-tab" data-tab="display">🎨 <?php _e('Display', 'readinizer-pro'); ?></a>
                <a href="#style" class="nav-tab" data-tab="style">✨ <?php _e('Style', 'readinizer-pro'); ?></a>
                <a href="#analytics" class="nav-tab pro-tab" data-tab="analytics">📊 <?php _e('Analytics', 'readinizer-pro'); ?></a>
                <a href="#templates" class="nav-tab pro-tab" data-tab="templates">🔧 <?php _e('Templates', 'readinizer-pro'); ?></a>
                <a href="#progress" class="nav-tab pro-tab" data-tab="progress">📈 <?php _e('Progress Bars', 'readinizer-pro'); ?></a>
            </div>
                ...
                <div class="readinizer-pro-main">
                    ...
                    <div id="tab-general" class="tab-content active">
                        <form method="post" action="options.php">
                            <?php
                            settings_fields('readinizer_pro_settings');
                            do_settings_sections('readinizer-pro');
                            ?>
                            ...
                            <div class="preview-section">
                                ...
                                <div id="readinizer-preview" class="preview-box">
                                    <?php 
                                    $sample_stats = array('reading_time' => 3, 'word_count' => 650);
                                    echo $this->generate_reading_time_html($sample_stats);
                                    ?>
                                </div>
                            </div>
                    ...
                    <div id="tab-analytics" class="tab-content">
                        <?php if ($this->analytics) echo $this->analytics->render_analytics_dashboard(); ?>
                    </div>

                    <!-- Templates Tab -->
                    <div id="tab-templates" class="tab-content">
                        <?php if ($this->templates) echo $this->templates->render_templates_page(); ?>
                    </div>

                    <!-- Progress Bars Tab -->
                    <div id="tab-progress" class="tab-content">
                        <?php if ($this->progress_bars) echo $this->progress_bars->render_progress_settings(); ?>
                    </div>
                </div>

                <div class="readinizer-pro-sidebar">
                    <div class="readinizer-pro-box wpezo-support">
                        <h3>💜 <?php _e('WPezo Plugins', 'readinizer-pro'); ?></h3>
                        <p><?php _e('Discover more professional WordPress plugins and themes by WPezo.', 'readinizer-pro'); ?></p>
                        <a href="<?php echo READINIZER_PRO_BRAND_URL; ?>" target="_blank" class="button">
                            <?php _e('Explore WPezo Products', 'readinizer-pro'); ?>
                        </a>
                    </div>

                    <div class="readinizer-pro-box">
                        <h3>📖 <?php _e('How to Use', 'readinizer-pro'); ?></h3>
                        <ul>
                            <li><?php _e('Enable automatic display in General Settings', 'readinizer-pro'); ?></li>
                            <li><?php _e('Choose post types to display on', 'readinizer-pro'); ?></li>
                            <li><?php _e('Use shortcodes for manual placement:', 'readinizer-pro'); ?></li>
                        </ul>
                        <p>
